...
const createTeachervalidation = async function (req, res, next) {
    try {
        ...
        if (!isValidFullName(fullname)) {
            return res.status(422).send({ status: 1002, message: "Please provide a valid fullName" })
        }
        ...
        if (!isValidGender(gender)) {
            return res.status(422).send({ status: 1002, message: "Invalid gender" })
        }
        ...
        if (!isValidAddress(address)) {
            return res.status(422).send({ status: 1002, message: "Invalid Address, Please enter address in a correct format" })
        }
        ...
        if (!isvalidBirthdate(dob)) {
            return res.status(422).send({ status: 1002, message: "Please enter date of birth from the past Only, It cannot current year or greater from the current year" })
        }
        ...
        if (!validateDate(joiningdate)) {
            return res.status(422).send({ status: 1002, message: "Invalid Joining Date or Please enter date of joining in the correct format" })
        }
...
export {createTeachervalidation}
